  ...
  return (
    <main className="app doc">
      ...
      <p>
        ...
        investigation so you can see the agent think, verify, and seal a brief — without any account
        or API key.
      </p>
...
npm run dev                         # http://localhost:3000`}</code>
      ...
      <p>
        <strong>Recon</strong> (default) — private-tier Venice models with web search, scraping, and
        function calling. This is where the legwork happens.
      </p>
      <p>
        <strong>Vault</strong> — a final, tool-free synthesis routed through an end-to-end-encrypted
        (<code>e2ee-</code>) model with <code>enable_e2ee</code>. The receipt reports the value Venice
        echoes back, so privacy is never over-claimed.
      </p>
      ...
      <ul>
        ...
        <li>
          <code>e2ee_applied</code> — read from the echoed <code>venice_parameters.enable_e2ee</code>
          , not assumed
        </li>
        <li>
          <code>attestation</code> — <code>pending</code> until the live TEE attestation endpoint is
          verified; never claimed as proven before then
        </li>
      </ul>
      ...
      <p>
        ...
        Server-Sent Events stream of <code>status</code>, <code>activity</code>, <code>receipt</code>
        , <code>plan</code>, <code>dossier</code>, <code>vault</code>, <code>cover</code>, and{" "}
        <code>audio</code> events (terminated by <code>data: [DONE]</code>). Optional body fields:{" "}
        <code>veniceApiKey</code>, <code>deep</code>, <code>model</code>, <code>persona</code>.
      </p>
      <h3>MCP server</h3>
      <p>
        Run <code>npm run mcp</code> to expose a <code>research</code> tool over stdio — drop it into
        Cursor, Claude, or any MCP client.
      </p>
      <h3>Venice Skill</h3>
      <p>
      ...
      <p>
        <strong>Is it free?</strong> The app is free to host; inference is funded by you (BYOK) or by
        a wallet (x402). The demo needs nothing.
      </p>
      <p>
